...
def db_connect():
    if current_app.config['DB_TYPE'] == 'postgres':
        conn = psycopg2.connect(
            dbname="artyom_zaharov_knowledge_base",
...
def put_film(id):
    ...
    if not film.get('title_ru'):
        return jsonify({'error': 'Русское название должно быть непустым.'}), 400
    if not film.get('title') and not film['title_ru']:
        return jsonify({'error': 'Название на оригинальном языке должно быть непустым, если русское название пустое.'}), 400
    current_year = datetime.now().year
    if not (1895 <= film.get('year', 0) <= current_year):
        return jsonify({'error': f'Год должен быть от 1895 до {current_year}.'}), 400
